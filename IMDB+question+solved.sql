...
-- Here we can see that the all different genres in table genre.
...
WITH one_genre 
AS (SELECT movie_id, count(DISTINCT genre) AS n_genre
	FROM genre
	GROUP BY movie_id
    HAVING n_genre = 1)
SELECT Count(*) AS 'Number of movies with only 1 genre'
FROM one_genre;

-- Here 3289 are the number of movies with only one genre.
...
-- There are total 361 movies published between 2018-04-01 to 2019-04-01 with median raing 8.
...
-- In both the cases, German movies got more votes than Italian movies.
...
(SELECT n.NAME AS director_name,
 ...
 ON dm.name_id = n.id
 INNER JOIN ratings r
 ON g.movie_id = r.movie_id, top_genres
 WHERE avg_rating > 8
...
-- Converting values in INR to dollars after the data-type is corrected using conversion equation, 1 USD = 75 INR
movie_income AS
...
WHERE prod_comp_rank <= 2;
 ...
 -- Here we can say that Star Cinema and Twentieth Century Fox are the top two production houses that have produced the highest number of hits among multilingual movies.
...
/* WE didn't use limit clause here as the 4th actress have also the same rating and same movie count 
Our report will become biased if we used LIMIT clause here. */
